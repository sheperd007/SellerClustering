...
def Preprocess_category_Data(Data,col_remove,number,log_col,box=False,Level="Category Level 1"):
    '''
    ...
    if Level== "Category_Group (Vertical)" :
        Categoriy_Data=Data[Data.Category_Group_Id==number][:] 
...
def Clustering_Kmeans(Data,n_clusters,col_remove,Mean_for_unscaling,Std_for_unscaling,log_col,test_K=True,fixed_size=None,Random_State=None,Key='SellerID'):
    """
    ...
    clusters=pd.concat([Data.loc[Data.drop(col_remove,axis=1).dropna().index], pd.DataFrame({'cluster':labels}).set_index(Data.drop(col_remove,axis=1).dropna().index)], axis=1)
    # UnScaling
    ...
    cen=clusters.groupby('cluster').agg({'%s' %(Key):'count'}).reset_index().join(centroid)
    cen=cen.rename(columns={"%s" %(Key): "Count"})
    return [clusters,cen]

def Clustering_KMedoids(Data,n_clusters,col_remove,Mean_for_unscaling,Std_for_unscaling,log_col,test_K=True,fixed_size=None,Random_State=None,Key='SellerID'):
    """
...
def df_to_db(df,server,database,Schema,table,strategy_if_exist='fail', chunksize=5):
    ...
    db_params = urllib.parse.quote_plus(r'DRIVER={SQL Server};SERVER='+server+';DATABASE='+database+';Trusted_Connection=yes')
    engine = sqlalchemy.create_engine("mssql+pyodbc:///?odbc_connect={}".format(db_params))

    df.to_sql(table, engine,schema=Schema,index=False, if_exists=strategy_if_exist, chunksize=chunksize,method='multi' )
