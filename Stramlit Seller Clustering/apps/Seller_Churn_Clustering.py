# -*- coding: utf-8 -*-
"""
Created on Tue Aug 24 15:13:06 2021
...
def app():
    ...
    We try to cluster sellers according to the available features for Them and respond to the needs of the Commercial team and the business intelligence team.
    
    ***
    """)
    
    st.markdown("""First, the user must select the execution time of the sellers' churn model, then according to this time, the output of the churn model is extracted, and then the user must specify the desired limits for the churn probability.
In other words, at this stage, the user must specify the alpha and beta values, for which the data of sellers that satisfy the term $alpha<P(SellerBecameChurn)<beta$ is called for clustering.""")
    ######################
    #Functions
    ######################
    ...
    def load(Level="Category Level 1"):
        if Level=="Category Level 1":
            Data=get_data_set('seller_clustering_Limited_in_time')
        if Level== "Category_Group (Vertical)" :
            Data=get_data_set('seller_clustering_vertical_Limited_in_time')
    ...
    #Read Times That Seller Churn Model Had Run
    conn=conncet_to_server()
    query = """select PersianDayMonthYear,insert_date from (select DISTINCT insert_date from 'database'.'schema'.[seller_churn_predict]) as a
    join DWSNDigiKala.dbo.vwDateAll as d on d.DateString=a.insert_date """
    Insert_date = pd.read_sql_query(query, conn)
    conn.close()
    available_dates_for_churn=list(Insert_date.PersianDayMonthYear)
    
    churn_date=st.sidebar.selectbox("The desired time for the churn model", Insert_date)
    
    #Threshold that data is must be filter according on that  ie: alpha<P(Seller become Churn)<Beta
    alpha=st.sidebar.slider("Alpha",0.0,1.0,0.0,0.01,key="1")
    Beta=st.sidebar.slider("Beta",0.0,1.0,0.5,0.01,key="2")
     
    #Load SellerID's that Satisfy alpha<P(Seller become Churn)<Beta
    conn=conncet_to_server()
    query = """SELECT SellerID
      FROM 'database'.'schema'.[seller_churn_predict]
      where insert_date='{}' and churn_prob between {} and {}""".format(list(Insert_date[Insert_date.PersianDayMonthYear==churn_date].insert_date)[0],alpha,Beta)
    SellerIDs = pd.read_sql_query(query, conn)
    conn.close()
    
    SellerIDs=list(SellerIDs.SellerID)
    
    #Select Granularity Level for Analysis 
    granularity_level=st.sidebar.selectbox("granularity level", ["put your granularity here"])
    
    Data_loaded=load(Level=granularity_level)
    Data=Data_loaded[Data_loaded.SellerID.isin(SellerIDs)]
    ...
    ***
    ...
    if granularity_level== "Category_Group (Vertical)":
         col_remove=["removable columns"]
    log_col=["columns we want to logarithm"]
    @st.cache
    ...
    for i in ["columns we want to logarithm"]:
